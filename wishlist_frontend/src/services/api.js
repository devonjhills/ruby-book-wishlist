...
export const itemsAPI = {
  getItems: (params = {}) => {
    railsActivityTracker?.addActivity('load_dashboard', 'Loading user items with filters and sorting');
    return api.get('/items', { params });
  },
  getItem: (id) => api.get(`/items/${id}`),
  createItem: (itemData) => {
    railsActivityTracker?.addActivity('create_book', `Creating new book: ${itemData.title}`);
    return api.post('/items', { item: itemData });
  },
  updateItem: (id, itemData) => {
    railsActivityTracker?.addActivity('update_book', `Updating book with ID: ${id}`);
...
export const searchAPI = {
  searchBooks: (query) => {
    railsActivityTracker?.addActivity('search_books', `Searching for books: "${query}"`);
